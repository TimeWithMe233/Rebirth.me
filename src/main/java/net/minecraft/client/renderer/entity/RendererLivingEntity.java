...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    protected HashMap<T,List<RenderData>> map = new HashMap<>();


    public void renderFake(final T entity,RenderData data){
        if(Minecraft.getMinecraft().currentScreen != null && entity instanceof EntityPlayerSP){
            return;
        }


        double x = data.getTempX() - renderManager.renderPosX;
        double y = data.getTempY() - renderManager.renderPosY;
        double z = data.getTempZ() - renderManager.renderPosZ;
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z))) {
            ...
            this.mainModel.swingProgress = data.getSwingProgress();

            try {

                this.renderLivingAt(entity, x, y, z);
                this.rotateCorpse(entity, data.getF8(), data.getF(), data.getPartialTicks());
                GlStateManager.enableRescaleNormal();
                GlStateManager.scale(-1.0F, -1.0F, 1.0F);
                this.preRenderCallback(entity, data.getPartialTicks());
                final float f4 = 0.0625F;
                GlStateManager.translate(0.0F, -1.5078125F, 0.0F);
                GlStateManager.enableAlpha();
                this.mainModel.setLivingAnimations(entity, data.getF6(), data.getF5(), data.getPartialTicks());
                this.mainModel.setRotationAngles(data.getF6(), data.getF5(), data.getF8(), data.getF2(), data.getF7(), 0.0625F, entity);

                if (CustomEntityModels.isActive()) {
                    this.renderEntity = entity;
                    this.renderLimbSwing = data.getF6();
                    this.renderLimbSwingAmount = data.getF5();
                    this.renderAgeInTicks = data.getF8();
                    this.renderHeadYaw = data.getF2();
                    this.renderHeadPitch = data.getF7();
                    this.renderScaleFactor = f4;
                    this.renderPartialTicks = data.getPartialTicks();
                }

                if (this.renderOutlines) {
                    final boolean flag1 = this.setScoreTeamColor(entity);
                    this.renderModel(entity, data.getF6(), data.getF5(), data.getF8(), data.getF2(), data.getF7(), 0.0625F);

                    if (flag1) {
                        this.unsetScoreTeamColor();
                    }
                } else {
                    final boolean flag = entity.render && this.setDoRenderBrightness(entity, data.getPartialTicks());
                    ...
                    if (EmissiveTextures.isActive()) {
                        if (EmissiveTextures.hasEmissive()) {
                            ...
                            GlStateManager.pushMatrix();
                            this.renderModel(entity, data.getF6(), data.getF5(), data.getF8(), data.getF2(), data.getF7(), f4);
                            GlStateManager.popMatrix();
...
//            if (!this.renderOutlines) {
//                super.doRender(entity, x,y, z, data.getEntityYaw(), data.getPartialTicks());
//            }
...
//            }
//            ArrayList<RenderData> list = (ArrayList) map.get(entity);
//            if(list.size() >= 301){
//                renderFake(entity,list.get(0));
//                renderFake(entity,list.get(100));
//                renderFake(entity,list.get(200));
//            }
//        }
...
//                if(this instanceof RenderPlayer){
//                    float tempX = (float) (x + renderManager.renderPosX);
//
//                    float tempY = (float) (y + renderManager.renderPosY);
//                    float tempZ = (float) (z + renderManager.renderPosZ);
//                    ArrayList<RenderData> list = (ArrayList) map.get(entity);
//
//
//                    list.add(new RenderData(tempX,tempY,tempZ,this.getSwingProgress(entity, partialTicks),partialTicks,entityYaw,f,f2,f5,f6,f7,f8));
//                    if(list.size() > 301){
//                        list.remove(0);
//                    }
//                }
...
@Getter
class RenderData{
    private float tempX,tempY,tempZ,swingProgress,partialTicks,entityYaw,f,f2,f5,f6,f7,f8;

    public RenderData(float x, float y, float z, float swingProgress, float partialTicks, float entityYaw, float f, float f2, float f5, float f6, float f7, float f8) {
        this.tempX = x;
        this.tempY = y;
        this.tempZ = z;
