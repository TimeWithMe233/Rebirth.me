...
import com.alan.clients.util.shader.impl.FileUtils;
...
public class ShaderUtil2 implements InstanceAccess {
    ...
    public ShaderUtil2(String fragmentShadersrc, boolean notUsed) {
        int program = glCreateProgram();
        int fragmentShaderID = createShader(new ByteArrayInputStream(fragmentShadersrc.getBytes()), GL_FRAGMENT_SHADER);
        int vertexShaderID = 0;
        try {
            vertexShaderID = createShader(mc.getResourceManager().getResource(new ResourceLocation("lavender/shader/tenacity/vertex.vsh")).getInputStream(), GL_VERTEX_SHADER);
        } catch (IOException e) {
    ...
    public ShaderUtil2(String fragmentShaderLoc) {
        this(fragmentShaderLoc, "rebirth/shader/tenacity/vertex.vsh");
    }
